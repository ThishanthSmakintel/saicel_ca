...
@section('dashboardContent')
        ...
        <div class="col-lg-3 col-md-6 col-sm-6">
            <div class="custom-icon-card card card-stats">
                <div class="card-header card-header-success card-header-icon">
                    ...
                    <p class="card-category">Most Visited Pages</p>
                    <ul id="mostVisitedPages" class="card-title">Loading...</ul>
                </div>
                <div class="card-body">
                    <div class="stats">
                        <div class="progress" style="height: 4px">
        ...
        <div class="col-lg-3 col-md-6 col-sm-6">
            <div class="custom-icon-card card card-stats">
                <div class="card-header card-header-danger card-header-icon">
                    ...
                    <p class="card-category">Most Visited Products</p>
                    <ul id="mostVisitedProducts" class="card-title">Loading...</ul>
                </div>
                <div class="card-body">
                    <div class="stats">
                        <div class="progress" style="height: 4px">
    ...
    <script>
        $(document).ready(function() {
            const visitorCountEventSource = new EventSource('{{ $sseVisitorCountUrl }}');
            visitorCountEventSource.onmessage = function(event) {
                const data = JSON.parse(event.data);
                $('#visitorCount').text(data.visitorCount);
            };

            const mostVisitedPagesEventSource = new EventSource('{{ $sseMostVisitedPagesUrl }}');
            mostVisitedPagesEventSource.onmessage = function(event) {
                ...
                mostVisitedPages.forEach(page => {
                    pagesHtml += `<li>${page.page_visited} - ${page.visits} visits</li>`;
                });
                $('#mostVisitedPages').html(pagesHtml);
            };

            const mostVisitedProductsEventSource = new EventSource('{{ $sseMostVisitedProductsUrl }}');
            mostVisitedProductsEventSource.onmessage = function(event) {
                const data = JSON.parse(event.data);
                const mostVisitedProducts = data.mostVisitedProducts;
                let productsHtml = '';
                mostVisitedProducts.forEach(product => {
                    productsHtml +=
                        `<li>Product ID: ${product.product_id} - ${product.visits} visits</li>`;
                });
                $('#mostVisitedProducts').html(productsHtml);
            };
